...
|---|---|---|---|
| **v2.8.6** | 2026-09-18 | `-` | **(TERKINI)** Form puzzle: **teks pembuka** per event (`pzIntro`, diedit di builder; paragraf dipisah baris kosong, baris pertama tebal, salam penutup lebih kecil). Bila ada teks pembuka, pertanyaan jadi label kolom. Label nama → **"Nama Lengkap"**. Preset 300 keping (25×12) di builder. Event HKA: 300 keping, pertanyaan "Satu Kata untuk Hari Keluarga Astra 2026". |
| **v2.8.5** | 2026-09-17 | `-` | **Fix tombol QR & Link.** Gambar QR kosong karena semua QR bergantung layanan luar `api.qrserver.com` yang gagal dimuat di browser. Sekarang QR dibuat **lokal di browser** oleh `qrcode.js` (qrcode-generator 1.4.4, Kazuhiko Arase, lisensi MIT, disimpan di situs sendiri; layanan luar hanya cadangan). Berlaku di 4 tempat: popup QR & Link, kartu dashboard, link Hasil, dan QR layar live. Popup kini punya tombol ✕ Tutup, 📋 Salin Link, bisa ditutup dengan Esc, dan di layar live latar digelapkan dengan QR besar. File `qr-PUZZLE.png` dihapus. |
| **v2.8.4** | 2026-09-17 | `-` | Layar puzzle **layar penuh**: header & footer dibuang, gambar puzzle ±90% lebar layar, kolom kanan 10% berisi logo, penghitung keping, QR kecil + pertanyaan, dan tombol host. Tombol **⛶ layar penuh** (Fullscreen API). Baris "Kode PUZZLE" di bawah QR dihapus (tak berguna di acara: peserta tak tahu alamat situs untuk mengetik kode). HP: kolom kanan jadi baris atas. |
| **v2.8.3** | 2026-09-17 | `-` | Pertanyaan puzzle kini diatur per event (`pzPrompt`, isi di builder). Event HKA: **"1 Kata Untuk Astra"** — tampil besar di form HP & di kartu QR layar utama; input jadi satu baris pendek (maks 60 huruf); jawaban pendek muncul besar di popup agar terbaca dari jauh; kolom Excel mengikuti pertanyaan. |
| **v2.8.2** | 2026-09-17 | `-` | Puzzle HKA siap acara: gambar teaser asli terpasang (dikompres 1920×1080, nama acak); jarak kisi mengecil otomatis di keping banyak (1px di 400 keping, sebelumnya 3px menutupi gambar); QR bisa pakai file statis di situs sendiri (`qrImg`) dengan layanan luar sebagai cadangan, agar tak bergantung pihak ketiga di Wi-Fi venue; judul kartu QR puzzle "SCAN & TULIS TESTIMONI". |
| **v2.8.1** | 2026-09-16 | `-` | Puzzle: jumlah keping kini bisa sampai **40×40** — preset cepat (24/80/192/400) + input manual kolom×baris dengan peringatan bila keping banyak (butuh banyak peserta / pakai Ungkap Semua). Event HKA di-set ke 400 keping (25×16). |
| **v2.8** | 2026-09-16 | `-` | **Dua fitur.** (1) **Export Excel rapi** — tombol unduh kini menghasilkan `.xlsx` asli berformat (murni JS, tanpa library: stored-zip + CRC32), 3 sheet (Rekap+Peringkat, Detail per Responden, Masukan Kualitatif) menggantikan CSV mentah. (2) **Puzzle Reveal HKA** — jenis sesi baru `puzzle`: peserta scan → wajib tulis testimoni → tiap testimoni membuka 1 keping (1 HP 1 keping, transaksi server anti-dobel) dari gambar HKA-1 ke HKA-2 (lokasi tersembunyi); animasi flip 3D + testimoni melintas lalu jatuh & hilang; host punya tombol **🔓 Ungkap Semua**, unduh testimoni, reset. Placeholder `hka1.png`/`hka2.png` disertakan (tinggal diganti). |
| **v2.7** | 2026-07-21 | `-` | **Link "Lihat Hasil" read-only untuk dibagikan.** Route baru `#/r/CODE` menampilkan halaman Hasil kuesioner ke siapa pun (atasan) **tanpa login & tanpa masuk form** — kontrol pemilik (Edit/Reset) disembunyikan, Export CSV tetap ada. Di halaman Hasil host ada tombol **🔗 Bagikan Hasil ke Atasan** (link + QR + salin). Catatan: siapa pun yang punya link bisa melihat. |
| **v2.6.1** | 2026-07-21 | `-` | **Fix bug submit kuesioner matriks.** `getFieldValue` langsung `return ""` bila elemen `fi_<fid>` tidak ada — padahal matriks memang tak punya elemen itu (sel-selnya ber-id `m_/mo_/mr_`), sehingga isian Bagian A **selalu terbaca kosong** → pesan "Lengkapi: BAGIAN A" muncul walau sudah diisi. Cabang matriks kini diproses **sebelum** guard `fi_`. Mengisi 1–2 vendor saja (sesuai yang pernah dipakai) kini sah. |
| **v2.6** | 2026-07-20 | `-` | **Ripple di semua halaman.** Efek sonar dari ikon Q kini juga tampil di logo QUERY pada landing, dashboard, dan hero setiap sesi (Q&A/kuesioner/vote) — varian **ring biru Astra** (`.qripple.qblue`) karena halaman-halaman itu berlatar terang; layar live tetap ring putih. Wrapper `.qlogo-wrap` menjaga pusat ripple tepat di ikon Q. |
| **v2.5** | 2026-07-16 | `-` | **Subtitle putih + ripple animasi.** Aset baru `query-logo-white.png` (dibuat dari `query-logo.png`): semua putih & **alpha subtitle "Suara Insan Astra" dinaikkan 63→255** (aslinya cuma 25% opacity, itu sebabnya tampak abu-abu — filter CSS tak bisa menaikkan alpha). Filter `brightness/invert` tak lagi dipakai. **Ripple "sonar" beranimasi** memancar dari ikon Q (3 ring, delay bertahap, dari belakang logo). File asli `query-logo.png` tidak diubah (masih dipakai halaman terang). |
| **v2.4** | 2026-07-16 | `-` | **Logo QUERY di layar live**: kotak *frosted glass* dihapus (termasuk `backdrop-filter:blur(22px)` yang bikin logo lembut/kurang tajam); logo dijadikan **putih (knockout)** agar kontras di panel gelap — PNG aslinya biru gelap `RGB(0,60,116)`; ukuran diperbesar **56→76px** (desktop) & **30→46px** (mobile). Catatan: PNG sudah 800×304 (resolusi tinggi), jadi masalahnya bukan resolusi. |
