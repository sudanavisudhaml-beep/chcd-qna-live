...
|---|---|---|---|
| **v2.8.5** | 2026-09-17 | `-` | **(TERKINI)** **Fix tombol QR & Link.** Gambar QR kosong karena semua QR bergantung layanan luar `api.qrserver.com` yang gagal dimuat di browser. Sekarang QR dibuat **lokal di browser** oleh `qrcode.js` (qrcode-generator 1.4.4, Kazuhiko Arase, lisensi MIT, disimpan di situs sendiri; layanan luar hanya cadangan). Berlaku di 4 tempat: popup QR & Link, kartu dashboard, link Hasil, dan QR layar live. Popup kini punya tombol ✕ Tutup, 📋 Salin Link, bisa ditutup dengan Esc, dan di layar live latar digelapkan dengan QR besar. File `qr-PUZZLE.png` dihapus. |
